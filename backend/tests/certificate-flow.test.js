const test = require('node:test');
const assert = require('node:assert/strict');
const fs = require('node:fs');
const os = require('node:os');
const path = require('node:path');
const { CertificateStore } = require('../src/services/certificateStore');

test('issues, looks up, and revokes a certificate', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'certicheck-test-'));
  const storageFile = path.join(tempDir, 'certificates.json');
  const store = new CertificateStore({ storageFile });

  const issued = await store.issue({
    certificateId: 'CERT-TEST-001',
    holderName: 'Ada Lovelace',
    holderEmail: 'ada@example.com',
    certificateType: 'Degree Certificate',
    issuerName: 'Certicheck',
    issuerWallet: 'issuer-wallet',
    ipfsCid: 'ipfs-test-cid',
    blockchainTransactionId: 'tx-test-001',
    userId: 42
  });

  assert.equal(issued.certificate_id, 'CERT-TEST-001');
  assert.equal(issued.verification_status, 'valid');

  const found = await store.lookup('CERT-TEST-001');
  assert.ok(found);
  assert.equal(found.certificate_id, 'CERT-TEST-001');

  const revoked = await store.revoke('CERT-TEST-001', 'Test revoke', 99);
  assert.equal(revoked.verification_status, 'revoked');

  const afterRevoke = await store.lookup('CERT-TEST-001');
  assert.equal(afterRevoke.verification_status, 'revoked');

  fs.rmSync(tempDir, { recursive: true, force: true });
});
